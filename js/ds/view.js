/* ============================================
   ...
   Tab-based page for all DS visualizations
   ============================================ */

const DSView = {
    engine: null,
    currentDS: 'linked-list',
    resizeHandler: null,

    mount(container) {
        this.engine = new AlgoEngine();
        this.engine.baseDelay = 300;
    ...
    _renderOperations() {
        ...
        switch (this.currentDS) {
          ...
          <div class="op-input-group">
            <input type="number" id="ds-value" class="brutal-input" placeholder="Value" value="42">
          </div>
          <div class="op-input-group">
            <input type="number" id="ds-index" class="brutal-input" placeholder="Index" value="0" min="0" style="width:70px">
          </div>
        ...
        `;
          ...
          <div class="toolbar-separator"></div>
          <select id="heap-type" class="brutal-select">
    ...
    _initCurrentDS() {
        ...
        switch (this.currentDS) {
            case 'linked-list':
                LinkedListViz.init(canvas);
                break;
            case 'stack':
                StackQueueViz.init(canvas, 'stack');
                break;
            case 'queue':
                StackQueueViz.init(canvas, 'queue');
                break;
            case 'binary-tree':
                BinaryTreeViz.init(canvas);
                break;
            case 'heap':
                HeapViz.init(canvas);
                break;
    ...
    _executeOp(op) {
        const valueInput = document.getElementById('ds-value');
        const value = valueInput ? parseInt(valueInput.value) || 0 : 0;
        const indexInput = document.getElementById('ds-index');
        const index = indexInput ? parseInt(indexInput.value) || 0 : 0;
        let steps = [];

        this.engine.stop();
        this._updatePlayPause(false);
        const viz = this._getViz();
        if (viz.reset) viz.reset();
        if (viz.resetHighlights) viz.resetHighlights();
        ...
        if (steps.length > 0) {
            ...
            document.getElementById('ds-timeline').max = steps.length - 1;
            document.getElementById('ds-timeline').value = 0;
            this.engine.play();
    ...
    _bindEvents() {
        ...
        document.querySelectorAll('.tab-item').forEach(tab => {
            tab.addEventListener('click', function () {
                ...
                self.currentDS = this.dataset.ds;
                self.engine.stop();
                self._updatePlayPause(false);

                // Destroy old viz
                const viz = self._getViz();
                if (viz && viz.destroy) viz.destroy();

                self._renderOperations();
                self._initCurrentDS();
                self._bindOpButtons();
                self._bindHeapType();

                document.getElementById('ds-timeline').value = 0;
                document.getElementById('ds-timeline').max = 0;
        ...
        document.getElementById('ds-speed').addEventListener('input', function () {
            ...
            document.getElementById('ds-speed-val').querySelector('.stat-value').textContent = speed + '×';
        });
        ...
        document.getElementById('ds-pause').addEventListener('click', () => {
            this.engine.pause();
            this._updatePlayPause(false);
        });
        ...
        document.getElementById('ds-step-fwd').addEventListener('click', () => {
            this.engine.stepForward();
        });
        document.getElementById('ds-step-back').addEventListener('click', () => {
            this.engine.stepBackward();
        });

        // Timeline
        document.getElementById('ds-timeline').addEventListener('input', function () {
            self.engine.pause();
            self._updatePlayPause(false);
        ...
        this.engine.onStep = (step, index) => {
            const viz = this._getViz();
            if (viz && viz.applyStep) viz.applyStep(step);
            document.getElementById('ds-timeline').value = index;
        ...
        this.engine.onReset = () => {
            const viz = this._getViz();
            if (viz && viz.reset) viz.reset();
            if (viz && viz.resetHighlights) viz.resetHighlights();
        };

        this.resizeHandler = () => {
            const viz = this._getViz();
            if (viz && viz._resize) viz._resize();
        };
    ...
    _bindOpButtons() {
        document.querySelectorAll('[data-op]').forEach(btn => {
            btn.addEventListener('click', () => {
                this._executeOp(btn.dataset.op);
            });
        });
    },

    _bindHeapType() {
        const heapType = document.getElementById('heap-type');
        if (heapType) {
            heapType.addEventListener('change', function () {
                HeapViz.setType(this.value === 'min');
            });
        }
    },

    _updatePlayPause(playing) {
        const playBtn = document.getElementById('ds-play');
        const pauseBtn = document.getElementById('ds-pause');
        if (playing) {
    ...
    _updateStepInfo(current, total) {
        const el = document.getElementById('ds-step-info');
