/* ============================================
   SORTING ALGORITHMS
   Each generator yields step events:
   { type: 'compare'|'swap'|'overwrite'|'sorted'|'partition'|'merge-split'|'bucket', indices, values, ... }
   ============================================ */

const SortingAlgorithms = {
    info: {
        'bubble': { name: 'Bubble Sort', time: 'O(n²)', space: 'O(1)', desc: 'Repeatedly swaps adjacent elements if they are in the wrong order.' },
        'selection': { name: 'Selection Sort', time: 'O(n²)', space: 'O(1)', desc: 'Finds the minimum element and places it at the beginning.' },
        'insertion': { name: 'Insertion Sort', time: 'O(n²)', space: 'O(1)', desc: 'Builds sorted array one element at a time by inserting each element into its correct position.' },
        'merge': { name: 'Merge Sort', time: 'O(n log n)', space: 'O(n)', desc: 'Divides array in half, sorts each half, then merges them.' },
        'quick': { name: 'Quick Sort', time: 'O(n log n)', space: 'O(log n)', desc: 'Picks a pivot and partitions elements around it.' },
        'heap': { name: 'Heap Sort', time: 'O(n log n)', space: 'O(1)', desc: 'Builds a max heap then repeatedly extracts the maximum.' },
        'radix': { name: 'Radix Sort', time: 'O(nk)', space: 'O(n+k)', desc: 'Sorts digit by digit from least to most significant.' },
        'counting': { name: 'Counting Sort', time: 'O(n+k)', space: 'O(k)', desc: 'Counts occurrences of each value and reconstructs the sorted array.' },
        'shell': { name: 'Shell Sort', time: 'O(n log²n)', space: 'O(1)', desc: 'Generalization of insertion sort that allows exchange of elements far apart.' },
    },

    /**
     * Run a named algorithm, returns array of steps
     * @param {string} name 
     * @param {number[]} arr - will be mutated
     * @returns {Array} steps
     */
    run(name, arr) {
        const steps = [];
        const a = [...arr]; // work on copy
        switch (name) {
            case 'bubble': this._bubble(a, steps); break;
            case 'selection': this._selection(a, steps); break;
            case 'insertion': this._insertion(a, steps); break;
            case 'merge': this._mergeSort(a, steps, 0, a.length - 1); break;
            case 'quick': this._quickSort(a, steps, 0, a.length - 1); break;
            case 'heap': this._heapSort(a, steps); break;
            case 'radix': this._radixSort(a, steps); break;
            case 'counting': this._countingSort(a, steps); break;
            case 'shell': this._shellSort(a, steps); break;
        }
        // Mark all as sorted at end
        for (let i = 0; i < a.length; i++) {
            steps.push({ type: 'sorted', indices: [i], array: [...a] });
        }
        return steps;
    },

    _bubble(a, steps) {
        const n = a.length;
        for (let i = 0; i < n - 1; i++) {
            for (let j = 0; j < n - i - 1; j++) {
                steps.push({ type: 'compare', indices: [j, j + 1], array: [...a] });
                if (a[j] > a[j + 1]) {
                    [a[j], a[j + 1]] = [a[j + 1], a[j]];
                    steps.push({ type: 'swap', indices: [j, j + 1], array: [...a] });
                }
            }
            steps.push({ type: 'sorted', indices: [n - i - 1], array: [...a] });
        }
    },

    _selection(a, steps) {
        const n = a.length;
        for (let i = 0; i < n - 1; i++) {
            let minIdx = i;
            for (let j = i + 1; j < n; j++) {
                steps.push({ type: 'compare', indices: [minIdx, j], array: [...a] });
                if (a[j] < a[minIdx]) {
                    minIdx = j;
                }
            }
            if (minIdx !== i) {
                [a[i], a[minIdx]] = [a[minIdx], a[i]];
                steps.push({ type: 'swap', indices: [i, minIdx], array: [...a] });
            }
            steps.push({ type: 'sorted', indices: [i], array: [...a] });
        }
    },

    _insertion(a, steps) {
        const n = a.length;
        for (let i = 1; i < n; i++) {
            let key = a[i];
            let j = i - 1;
            steps.push({ type: 'compare', indices: [i], array: [...a] });
            while (j >= 0 && a[j] > key) {
                steps.push({ type: 'compare', indices: [j, j + 1], array: [...a] });
                a[j + 1] = a[j];
                steps.push({ type: 'overwrite', indices: [j + 1], value: a[j], array: [...a] });
                j--;
            }
            a[j + 1] = key;
            steps.push({ type: 'overwrite', indices: [j + 1], value: key, array: [...a] });
        }
    },

    _mergeSort(a, steps, left, right) {
        if (left >= right) return;
        const mid = Math.floor((left + right) / 2);
        steps.push({ type: 'merge-split', indices: [left, mid, right], array: [...a] });
        this._mergeSort(a, steps, left, mid);
        this._mergeSort(a, steps, mid + 1, right);
        this._merge(a, steps, left, mid, right);
    },

    _merge(a, steps, left, mid, right) {
        const leftArr = a.slice(left, mid + 1);
        const rightArr = a.slice(mid + 1, right + 1);
        let i = 0, j = 0, k = left;
        while (i < leftArr.length && j < rightArr.length) {
            steps.push({ type: 'compare', indices: [left + i, mid + 1 + j], array: [...a] });
            if (leftArr[i] <= rightArr[j]) {
                a[k] = leftArr[i];
                i++;
            } else {
                a[k] = rightArr[j];
                j++;
            }
            steps.push({ type: 'overwrite', indices: [k], value: a[k], array: [...a] });
            k++;
        }
        while (i < leftArr.length) {
            a[k] = leftArr[i];
            steps.push({ type: 'overwrite', indices: [k], value: a[k], array: [...a] });
            i++; k++;
        }
        while (j < rightArr.length) {
            a[k] = rightArr[j];
            steps.push({ type: 'overwrite', indices: [k], value: a[k], array: [...a] });
            j++; k++;
        }
    },

    _quickSort(a, steps, low, high) {
        if (low < high) {
            const pi = this._partition(a, steps, low, high);
            steps.push({ type: 'sorted', indices: [pi], array: [...a] });
            this._quickSort(a, steps, low, pi - 1);
            this._quickSort(a, steps, pi + 1, high);
        } else if (low === high) {
            steps.push({ type: 'sorted', indices: [low], array: [...a] });
        }
    },

    _partition(a, steps, low, high) {
        const pivot = a[high];
        steps.push({ type: 'partition', indices: [high], pivot: pivot, array: [...a] });
        let i = low - 1;
        for (let j = low; j < high; j++) {
            steps.push({ type: 'compare', indices: [j, high], array: [...a] });
            if (a[j] < pivot) {
                i++;
                [a[i], a[j]] = [a[j], a[i]];
                steps.push({ type: 'swap', indices: [i, j], array: [...a] });
            }
        }
        [a[i + 1], a[high]] = [a[high], a[i + 1]];
        steps.push({ type: 'swap', indices: [i + 1, high], array: [...a] });
        return i + 1;
    },

    _heapSort(a, steps) {
        const n = a.length;
        // Build max heap
        for (let i = Math.floor(n / 2) - 1; i >= 0; i--) {
            this._heapify(a, steps, n, i);
        }
        // Extract elements
        for (let i = n - 1; i > 0; i--) {
            [a[0], a[i]] = [a[i], a[0]];
            steps.push({ type: 'swap', indices: [0, i], array: [...a] });
            steps.push({ type: 'sorted', indices: [i], array: [...a] });
            this._heapify(a, steps, i, 0);
        }
    },

    _heapify(a, steps, n, i) {
        let largest = i;
        const left = 2 * i + 1;
        const right = 2 * i + 2;

        if (left < n) {
            steps.push({ type: 'compare', indices: [left, largest], array: [...a] });
            if (a[left] > a[largest]) largest = left;
        }
        if (right < n) {
            steps.push({ type: 'compare', indices: [right, largest], array: [...a] });
            if (a[right] > a[largest]) largest = right;
        }
        if (largest !== i) {
            [a[i], a[largest]] = [a[largest], a[i]];
            steps.push({ type: 'swap', indices: [i, largest], array: [...a] });
            this._heapify(a, steps, n, largest);
        }
    },

    _radixSort(a, steps) {
        const max = Math.max(...a);
        for (let exp = 1; Math.floor(max / exp) > 0; exp *= 10) {
            this._countByDigit(a, steps, exp);
        }
    },

    _countByDigit(a, steps, exp) {
        const n = a.length;
        const output = new Array(n);
        const count = new Array(10).fill(0);

        for (let i = 0; i < n; i++) {
            const digit = Math.floor(a[i] / exp) % 10;
            count[digit]++;
            steps.push({ type: 'bucket', indices: [i], digit: digit, array: [...a] });
        }

        for (let i = 1; i < 10; i++) count[i] += count[i - 1];

        for (let i = n - 1; i >= 0; i--) {
            const digit = Math.floor(a[i] / exp) % 10;
            output[count[digit] - 1] = a[i];
            count[digit]--;
        }

        for (let i = 0; i < n; i++) {
            a[i] = output[i];
            steps.push({ type: 'overwrite', indices: [i], value: a[i], array: [...a] });
        }
    },

    _countingSort(a, steps) {
        const max = Math.max(...a);
        const count = new Array(max + 1).fill(0);
        const output = new Array(a.length);

        for (let i = 0; i < a.length; i++) {
            count[a[i]]++;
            steps.push({ type: 'bucket', indices: [i], digit: a[i], array: [...a] });
        }

        for (let i = 1; i <= max; i++) count[i] += count[i - 1];

        for (let i = a.length - 1; i >= 0; i--) {
            output[count[a[i]] - 1] = a[i];
            count[a[i]]--;
        }

        for (let i = 0; i < a.length; i++) {
            a[i] = output[i];
            steps.push({ type: 'overwrite', indices: [i], value: a[i], array: [...a] });
        }
    },

    _shellSort(a, steps) {
        const n = a.length;
        for (let gap = Math.floor(n / 2); gap > 0; gap = Math.floor(gap / 2)) {
            for (let i = gap; i < n; i++) {
                let temp = a[i];
                let j = i;
                while (j >= gap) {
                    steps.push({ type: 'compare', indices: [j - gap, j], array: [...a] });
                    if (a[j - gap] <= temp) break;
                    a[j] = a[j - gap];
                    steps.push({ type: 'overwrite', indices: [j], value: a[j - gap], array: [...a] });
                    j -= gap;
                }
                a[j] = temp;
                steps.push({ type: 'overwrite', indices: [j], value: temp, array: [...a] });
            }
        }
    }
};

window.SortingAlgorithms = SortingAlgorithms;
